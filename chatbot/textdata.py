...
class TextData:
    ...
    def __init__(self, args):
        ...
        self.corpusDir = os.path.join(self.args.rootDir, 'data/livechat/')
    ...
    def loadCorpus(self, dirName):
        ...
        if not datasetExist:  # First time we load the database: creating all files
            ...
            convObj = {}
            convObj["lines"] = []
            with open(self.corpusDir + 'dialog.csv', 'r') as f:
                reader = csv.reader(f)
                for row in reader:
                    convObj["lines"].append({'text': row[0].strip()})
                    convObj["lines"].append({'text': row[1].strip()})
